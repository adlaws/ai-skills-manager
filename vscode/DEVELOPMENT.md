...
| `github/resourceClient.ts` | Fetches resources from GitHub. **Full repos** use the Contents API to list category folders. **Skills repos** (`skillsPath` set) use the Git Trees API for recursive scanning. When `nestedSkills` is enabled, full repos also use the Trees API for the skills category, discovering `SKILL.md` files at any depth and computing group names from intermediate path segments. Raw content is fetched from `raw.githubusercontent.com`. All responses are cached in memory with a configurable TTL. Also fetches individual file SHAs for update detection and extracts tags from resource frontmatter. |
...
| `views/marketplaceProvider.ts` | Three-level `TreeDataProvider` (Repo → Category → Resource), with an optional fourth level (Repo → Category → SkillGroup → Resource) when `nestedSkills` is enabled. Supports search filtering, tag-based filtering, and tracks which items are already installed. Shows a Favorites section at the tree root when the user has starred resources. |
...
| `commands/installCommands.ts` | Registers install/uninstall commands: install (local and global), uninstall (single and bulk), check for updates, update single resource, update all, and Install All (installs all items under a category or skill group node). |
